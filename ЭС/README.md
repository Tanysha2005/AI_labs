...
## Инструментарий
Работа выполнялась в классической оболочке для разработки экспертных систем [CLIPS](https://sourceforge.net/projects/clipsrules/).

## Задание. Разработка ЭС "Выбор языка программирования"

**Описание задачи:** Спроектировать ЭС-консультант, которая на основе ответов пользователя (да/нет) выводит рекомендацию о том, какой язык программирования и специализацию в IT ему стоит выбрать.

**Структура ЭС (База знаний):**
Система использует продукционные правила (конструкция `defrule`). При запуске в рабочую память (Fact-list) помещается стартовый факт, который триггерит первый вопрос. Далее, считывая ввод пользователя (`read`), система добавляет новые факты (например, `(web yes)`), которые в свою очередь активируют следующие правила по цепочке. Это классическая реализация **прямого логического вывода (forward chaining)**.

### Тестирование программы и результаты:

Программа запускается последовательностью встроенных команд: `(load "expert.clp")` для загрузки базы знаний, `(reset)` для очистки рабочей памяти и установки стартовых фактов, `(run)` для запуска машины вывода.

1. **Ветвь: Веб-разработка -> Визуальный интерфейс.**
   * **Действие:** Запуск `(run)`. Ответы пользователя: `yes`, `yes`.
   * **Результат:** Экспертная система рекомендует JavaScript (Frontend).
...
2. **Ветвь: Веб-разработка -> Серверная часть.**
   ...
   * **Результат:** Экспертная система рекомендует Python или PHP (Backend).
...
3. **Ветвь: Не Веб -> Разработка игр.**
   ...
   * **Результат:** Система рекомендует C# (Unity) или C++ (Unreal Engine).
...
1. Познакомилась с программной средой **CLIPS** и её LISP-подобным синтаксисом (использование списков в круглых скобках).
...
4. Успешно разработала собственное дерево принятия решений для ЭС в предметной области "Выбор специализации в IT".
